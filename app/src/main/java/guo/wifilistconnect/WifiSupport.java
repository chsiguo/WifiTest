...
public class WifiSupport {
    ...
    public static WifiConfiguration createWifiConfig(String SSID, String password, WifiCipherType type) {
        Log.d("mypadd"," SSID: "+SSID+" password: "+password +" type: "+type);
...
//            config.wepTxKeyIndex = 0;
    ...
    public static boolean addNetWork(WifiConfiguration config, Context context) {
      ...
      //  Log.d("myconn", "启动连接：" + flag + "\n");
      //  Toast.makeText(context, "启动连接：" + config.SSID + " [" + flag + "]\n", Toast.LENGTH_LONG).show();
    ...
    //删除某个已经保存的Wifi热点：
    public static void RemoveConfig(String SSID,Context context){
        ...
        existingConfigs= mWifiManager.getConfiguredNetworks();
            for (WifiConfiguration existingConfig : existingConfigs) {
              if (existingConfig.SSID.equals("\"" + SSID + "\"")) {
                ...
                Toast.makeText(context, SSID + " 配置信息已经移除！ ",
                        Toast.LENGTH_LONG).show();
                  flag = false;
                }
            }
        if(flag) Toast.makeText(context, SSID + " 该AP信息没有配置！ ",
                                                    Toast.LENGTH_LONG).show();
    }
